...
$cls_Task->_add_step("Створити проект <b>task_1</b> у рішенні <b>Lesson_04</b>");
...
Тепер потрібно написати обробник події “натиснута кнопка”. Для цього знайдемо в списку подій об’єкта кнопки подію Click та виконаємо подвійний клік напроти неї. Ми перейшли в програмний код із заготовленим шаблоном обробника події btn_Calculate_Click. Цього самого можна досягти, виконавши подвійний клік на кнопці в конструкторі форм.
...
Тепер потрібно зчитати число з текстового поля tb_Input і присвоїти це число змінній Side. Для цього потрібно записати такий рядок:
...
зчитується текст з поля tb_Input;
...
Третій етап – Виведення результатів на екран. В елемент Напис можна виводити лише дані текстового типу. Тому, при потребі вивести в Напис числове значення, його потрібно попередньо перетворити в текст функцією <b>ToString()</b>:

<pre>
    <code class='cs hljs'>
        lb_Perimeter.Text = P.ToString();
        lb_Square.Text = S.ToString();
    </code>	
</pre>

Виконавши всі три етапи отримаємо метод <b>btn_Calculate_Click</b> – обробник події “натиснута кнопка”:
<pre>
    <code class='cs hljs'>
...
<i><b>Альтернативний варіант вигляду коду.</b></i>
...
<pre>
    <code class='cs hljs'>
        private void btn_Calculate_Click(object sender, EventArgs e)
        {
            byte Side = byte.Parse(textBox1.Text);
            int P = 4 * Side;
            int S = Side * Side;
            lb_Perimeter.Text = P.ToString();
            lb_Square.Text = S.ToString();
...
Перейдемо в конструктор форм та змінимо властивість Text написів на  \"Периметр = 0\" та \"Площа = 0\".
